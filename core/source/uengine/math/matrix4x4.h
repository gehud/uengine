...
namespace ue
{
	...
	public:
		...
		float operator [] (const vector2int& vector) const {
			UE_CORE_ASSERT(vector.x >= 0 && vector.x <= 3, "Row out of range.");
			UE_CORE_ASSERT(vector.y >= 0 && vector.y <= 3, "Column out of range.");
			return _values[vector.x][vector.y];
		}

		float& operator [] (const vector2int& vector) {
			UE_CORE_ASSERT(vector.x >= 0 && vector.x <= 3, "Row out of range.");
			UE_CORE_ASSERT(vector.y >= 0 && vector.y <= 3, "Column out of range.");
			return _values[vector.x][vector.y];
		}
